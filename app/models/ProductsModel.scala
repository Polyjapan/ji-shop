...
class ProductsModel @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext)
  ...
  def getMergedProducts(includeHidden: Boolean): Future[Seq[data.Product]] =
    db.run(productsJoin.filter(_._1.visible === true).filter(_._2.isVisible === true || includeHidden).result).map(_.map(_._2))
