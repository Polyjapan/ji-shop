...
import ch.japanimpact.api.tickets.TicketsAPI
...
/**
  * @author zyuiop
  */
class OrdersModel @Inject()(protected val dbConfigProvider: DatabaseConfigProvider, val ticketsApi: TicketsAPI)(implicit ec: ExecutionContext)
  ...
  def setOrderRemoved(orderId: Int, removed: Boolean) = {
    ...
    // TODO: call relevant code to remove the accred and ticket
  ...
  def loadOrder(orderId: Int, includeRemoved: Boolean = false): Future[Option[OrderData]] = {
    ...
    ??? // TODO: rewrite entirely the order data class and model
    /*
